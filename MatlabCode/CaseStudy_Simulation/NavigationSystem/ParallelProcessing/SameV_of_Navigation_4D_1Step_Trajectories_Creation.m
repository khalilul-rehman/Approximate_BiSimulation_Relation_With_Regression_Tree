...
parfor counterForSimulation = 1 : numberOfSimulations
    disp(['simulation #', int2str(counterForSimulation)]);
%parfor counterForSimulation = 1 : numberOfSimulations
    %% Initial Conditions
    % Initial Position
    x = [0.5; 1.5];
    ...
    v = [getRandomVelocity(0,3); getRandomVelocity(0,3)]; %-1 1
    ...
    trajectory = zeros(1,8);
    trajectory(1,1:4) = [ x'  v'];  % Store trajectory
    ...
    trajectories(counterForSimulation,:) = trajectory;
...
hold on;
grid on;
title('Navigation System Trajectories');
...
%parfor i = 1 : size(trajectories, 1)
    plot([trajectories(i,1); trajectories(i,5)], [trajectories(i,2); trajectories(i,6)], 'b-', 'LineWidth', 1);
    scatter([trajectories(i,1); trajectories(i,5)], [trajectories(i,2); trajectories(i,6)]);
end
hold off;   
...
function velocity_value = getRandomVelocity(min, max)
    velocity_value = (max-min).*rand(1,1) + min;
end
...
DataSet = [];
for i = 1 : size(trajectories,1)
    one_Trejectory = trajectories{i}(1:end,:)';
    size(one_Trejectory);
    ...
    DataSet = [DataSet ; [one_Trejectory nextStepState]];
...
%% 
   ...
   csvwrite('./Data_Files/4D_Data/OneStepSimulation/30k_R0_3/navigation_trejectory_dataset.csv',trajectories);
