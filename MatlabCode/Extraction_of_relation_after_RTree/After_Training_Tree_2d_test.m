...
%% 
    ...
    parentDirectory = fileparts(cd);
     ...
     dataFiles_folder_path = strcat(parentDirectory, '/CaseStudy_Simulation/NavigationSystem/Data_Files/4D_Data/OneStepSimulation/500000Sim/2Trees/');
...
if isempty(gcp('nocreate'))
    %parpool; % Starts a parallel pool with available workers
    parpool('local', 8); % Start a parallel pool with 4 workers
end
...
%% Temp
nrmses = zeros(numLeaves,1);
for i = 1 : numLeaves
    m = solutionOptimal_star{i}(1:4, 2:5);
    m0 = solutionOptimal_star{i}(1:4, end);
    y_predicted = (m * LAMBDA{i}')' + m0';
    y = lambda{i};
    % Compute squared error
    squared_error = sum((y - y_predicted).^2, 2); % Sum squared differences for each row
    
    % Compute mean squared error
    mse = mean(squared_error); % Average over all samples (N)
    
    % Compute root mean square error (RMSE)
    rmse = sqrt(mse);
    
    % Compute range of y
    y_range = max(y(:)) - min(y(:)); % Range across all elements of y
...
p_shape_of_y = 2; % Dimension_of_ClassVariables;
...
for i = 1 : numLeaves
    ...
    current_elevated_vertices = zeros(4,2);
